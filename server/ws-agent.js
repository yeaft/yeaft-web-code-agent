...
async function handleAgentMessage(agentId, msg) {
  ...
  //
  // fix-unify-workbench-file-content-drop: Workbench response types are
  // exempted from the conversation-id gate because Unify uses a virtual
  // `unify-<timestamp>` conversationId that is never registered in
  // `agent.conversations` (only Chat/Crew flows do that via
  // agent.conversations.set(...) in agent-conversation.js / agent-crew.js).
  // The real ownership boundary for these messages is `_requestUserId`
  // enforced by `forwardToClients` in ws-utils.js — the conversationId on
  // workbench responses is only echo, not an authorization key. Without
  // the exemption, `directory_listing` already worked (it's listed below)
  // but `file_content`, `file_saved`, `file_op_result`, `file_search_result`
  // were silently dropped here, breaking "open file" in the Unify workbench.
  const CONV_EXEMPT_TYPES = new Set([
    ...
    // Workbench responses — ownership-checked by _requestUserId downstream.
    // Files: read/write/list/search/delete/move/copy/create/upload.
    'file_content', 'file_saved', 'file_op_result', 'file_search_result',
    // Git: status/diff/add/reset/restore/commit/push.
