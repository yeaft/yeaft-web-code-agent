...
test.describe('Agent 断线重连', () => {
  test('agent 断开后重连，已有会话保留', async ({ chatPage, mockAgent }) => {
    ...
    // Agent offline in UI
    await expect(chatPage.locator('.brand-label')).toHaveText('0 agents', { timeout: 5000 });

    // Reconnect agent
    await mockAgent.reconnect();

    // Agent back online
    await expect(chatPage.locator('.brand-label')).not.toHaveText('0 agents', { timeout: 5000 });
