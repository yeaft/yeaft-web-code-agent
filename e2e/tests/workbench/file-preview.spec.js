...
// The image/load-error/operation-feedback markup is taken from FilesTab.template
...
function mountPreviewHarness() {
  ...
  const imageTemplate = section('<div v-else-if="activeFile.fileType === \'image\'"', '\n        </template>')
  ...
  const app = createApp({
    template: `<main class="preview-test-shell files-tab mobile-editor-view">
      <nav aria-label="Open images"><button v-for="(file, index) in openFiles" :key="file.path"
        @click="activeFileIndex = index" :aria-pressed="activeFileIndex === index">{{ file.name }}</button></nav>
      ${feedbackTemplate}
      <section class="file-col-content">
        ${loadingTemplate}<template v-else-if="activeFile">${imageTemplate}</template>
      </section>
    </main>`,
    setup() {
      const openFiles = ref([]);
      ...
      window.previewHarness = {
        ...
        open(path) {
          openFiles.value.push({
            path, name: path.split('/').pop(), fileType: 'image',
  ...
  async serve(request, response) {
    ...
    if (url.pathname.startsWith('/api/preview/')) {
      ...
      const body = id === 'missing' ? Buffer.from('Preview not found') : id === 'bad' ? Buffer.from('not an image')
        : id === 'large' ? LARGE_IMAGE : SMALL_IMAGE;
      const send = () => {
