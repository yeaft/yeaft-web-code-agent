/**
 * Server relay passthrough for `unify_dream_status` and `unify_dream_result`
 * (v0.1.758).
 *
 * The bug this guards against:
 *   The Yeaft Unify "Dream" feature has two manual trigger points in the UI
 *   (group conversation header + group settings modal). Both reach the
 *   agent's `handleUnifyDreamTrigger`, which emits two BARE top-level
 *   WebSocket messages: `unify_dream_status` (status: 'running') at the
 *   start of the run, and `unify_dream_result` (success/entriesCreated/
 *   lastDreamAt/groups/targets/error/skipped) at the end.
 *
 *   Before this fix, `server/handlers/agent-output.js` had cases for
 *   `unify_output` and `unify_history_chunk` but NO case for either dream
 ...
 *   "Running…" forever because `unify_dream_result` never arrived, even
 *   though the dream pass had actually completed on the agent.
 *
 * Strategy mirrors `unify-output-envelope.test.js`: mock the heavy deps,
...
describe('agent-output.js — unify_dream_status relay', () => {
...
describe('agent-output.js — unify_dream_result relay', () => {
