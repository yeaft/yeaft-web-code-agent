 ...
 * `SessionSettingsModal` exposes the `open-vp-library` emit and that its
 ...
 * the SessionSettingsModal end of the contract.
 */

import { describe, it, expect } from 'vitest';
import SessionSettingsModal from '../../web/components/SessionSettingsModal.js';

describe('SessionSettingsModal — open-vp-library wiring', () => {
  it('declares "open-vp-library" in emits', () => {
    expect(Array.isArray(SessionSettingsModal.emits)).toBe(true);
    expect(SessionSettingsModal.emits).toContain('open-vp-library');
    // 'close' must stay too (existing contract).
    expect(SessionSettingsModal.emits).toContain('close');
  });

  it('Members section renders an "Open VP Library" button that emits the event', () => {
    const tpl = String(SessionSettingsModal.template || '');
    // Section header layout wrapper.
    expect(tpl).toMatch(/group-settings-section-header/);
    // The button uses the new i18n key.
    expect(tpl).toMatch(/yeaft\.session\.members\.openLibrary/);
  ...
  it('hint text is wired as a title attribute for accessibility', () => {
    const tpl = String(SessionSettingsModal.template || '');
    expect(tpl).toMatch(/yeaft\.session\.members\.openLibraryHint/);
  });

  it('does not expose standalone per-group model configuration UI', () => {
    const sections = SessionSettingsModal.computed.sections.call({ $t: (key) => key });
    expect(sections.map(s => s.id)).not.toContain('model');
    expect(SessionSettingsModal.props.initialSection.validator('model')).toBe(false);
