/**
 * dream runner ↔ error-sink integration.
 *
 ...
 *      sink lands at `<root>/session/<id>/.dream-last-error.json` with
 *      `phase === 'triage'`.
 *
 ...
 *      `phase === 'apply'`. Critically, this is the NON-session-root scope
 *      case — it pins that the sink works for `user/`, `vp/<id>`,
 *      `feature/<id>` too, not just `session/<id>`.
 *
...
describe('runDream — error sink wiring', () => {
  it('writes session/<id>/.dream-last-error.json when triage throws', async () => {
    ...
    const r = await runDream({
      ...
      loadSessionDiff: async () => Array.from(
        { length: 25 },
    ...
    const sink = join(root, 'session', 'grp_fun', '.dream-last-error.json');
    expect(existsSync(sink)).toBe(true);
    const body = JSON.parse(readFileSync(sink, 'utf8'));
    expect(body.scope).toBe('session/grp_fun');
  ...
  it('writes user/.dream-last-error.json when an apply throws (non-group scope)', async () => {
    // Triage succeeds with default hard-rule actions (user + session/g +
    ...
    const llm = async ({ pass, prompt }) => {
      ...
      if (pass === 'update' || pass === 'create') {
        ...
        // session/g, vp/zhang-san) succeed so the runner reaches apply
        // for `user`.
    ...
    // Sink landed under the NON-session-root scope dir.
  ...
  it('overwrites a prior error sink on the next failed pass', async () => {
    ...
    const body = JSON.parse(
      readFileSync(join(root, 'session', 'g', '.dream-last-error.json'), 'utf8'),
    );
