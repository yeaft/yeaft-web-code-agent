...
import {
  ...
  buildCreatePrompt,
  targetToScope,
} from '../../../../agent/yeaft/dream/apply.js';
...
describe('targetToScope', () => {
  it('maps scope strings', () => {
    ...
    expect(targetToScope('session/g1')).toEqual({ kind: 'session', id: 'g1' });
    expect(targetToScope('session/g1/user')).toEqual({ kind: 'session-user', sessionId: 'g1' });
    expect(targetToScope('session/g1/vp/zhang')).toEqual({ kind: 'session-vp', sessionId: 'g1', id: 'zhang' });
    expect(() => targetToScope('session/g1/feature/f1')).toThrow(/deprecated|malformed/);
    expect(targetToScope('session/g1/topic/sci/phys'))
      .toEqual({ kind: 'session-topic', sessionId: 'g1', path: ['sci', 'phys'] });
  });
  it('rejects legacy root scopes', () => {
    expect(() => targetToScope('vp/zhang')).toThrow(/deprecated/);
    expect(() => targetToScope('feature/f1')).toThrow(/deprecated/);
    expect(() => targetToScope('topic/sci')).toThrow(/deprecated/);
  });
...
describe('UPDATE path', () => {
  ...
  it('batches when sources exceed cap, threading memory between batches', async () => {
    mkdirSync(join(root, 'session', 'g'), { recursive: true });
    writeFileSync(join(root, 'session', 'g', 'memory.md'), 'M0\n');
    writeFileSync(join(root, 'session', 'g', 'summary.md'), 'S0\n');
    ...
    const mem = readFileSync(join(root, 'session', 'g', 'memory.md'), 'utf8');
...
describe('CREATE path', () => {
  it('writes new files for a non-existent topic scope', async () => {
    ...
    const r = await applyMergedTarget(
      {
        target: 'session/g-eng/topic/science/physics',
        kind: 'create',
        sources: [{ sessionId: 'g-eng', diff: [{ role: 'user', body: 'hi' }] }],
      },
    ...
    const mem = readFileSync(join(root, 'session', 'g-eng', 'topic', 'science', 'physics', 'memory.md'), 'utf8');
    expect(mem).toContain('# physics');
    expect(mem).toContain('lastDreamAt: 2026-04-28T03:07:00Z');
  });

  it('downgrades CREATE to UPDATE when files already exist', async () => {
    mkdirSync(join(root, 'session', 'g', 'topic', 'science', 'physics'), { recursive: true });
    writeFileSync(join(root, 'session', 'g', 'topic', 'science', 'physics', 'memory.md'), 'PRESERVED\n');
    writeFileSync(join(root, 'session', 'g', 'topic', 'science', 'physics', 'summary.md'), 'old\n');
    const llm = async ({ pass }) => {
      expect(pass).toBe('update');
      return JSON.stringify({ memory_md: 'NEW\n', summary_md: 'new' });
    };
    await applyMergedTarget(
      { target: 'session/g/topic/science/physics', kind: 'create', sources: [{ sessionId: 'g', diff: [] }] },
      { root, ts: 'TS-5', llm },
    );
    expect(readFileSync(join(root, 'session', 'g', 'topic', 'science', 'physics', 'memory.md'), 'utf8'))
      .toContain('NEW');
  });
});
...
describe('prompt builders', () => {
  it('UPDATE prompt includes the current memory and source diffs', () => {
    ...
    expect(p).toContain('[session/g-eng]');
