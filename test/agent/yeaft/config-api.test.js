...
describe('global + agent LLM merge', () => {
  it('returns local agent config separately from effective global+agent config', () => {
    writeFileSync(join(TEST_DIR, 'config.json'), JSON.stringify({
      providers: [{ name: 'local', baseUrl: 'http://local/v1', apiKey: 'local-key', models: ['local-model'] }],
      primaryModel: 'local/local-model',
      fastModel: 'local/local-model'
    }));

    const result = getLlmConfig(TEST_DIR, {
      providers: [{ name: 'global', baseUrl: 'http://global/v1', apiKey: 'global-key', models: ['global-model'] }]
    });

    expect(result.agentConfig.providers.map(p => p.name)).toEqual(['local']);
    expect(result.effectiveConfig.providers.map(p => p.name)).toEqual(['global', 'local']);
    expect(result.effectiveConfig.providers[0].scope).toBe('global');
    expect(result.effectiveConfig.providers[1].scope).toBe('agent');
  });

  it('prefixes global provider names when they collide with local provider names', () => {
    writeFileSync(join(TEST_DIR, 'config.json'), JSON.stringify({
      providers: [{ name: 'same', baseUrl: 'http://local/v1', apiKey: 'local-key', models: ['local-model'] }]
    }));

    const result = getLlmConfig(TEST_DIR, {
      providers: [{ name: 'same', baseUrl: 'http://global/v1', apiKey: 'global-key', models: ['global-model'] }]
    });

    expect(result.effectiveConfig.providers.map(p => p.name)).toEqual(['global:same', 'same']);
    expect(result.effectiveConfig.providers[0].originalName).toBe('same');
  });

  it('does not persist global providers when updating local agent config', () => {
    writeFileSync(join(TEST_DIR, 'config.json'), JSON.stringify({ providers: [] }));

    const result = updateLlmConfig({
      providers: [{ name: 'local', baseUrl: 'http://local/v1', apiKey: 'local-key', models: ['m'] }]
    }, TEST_DIR, {
      providers: [{ name: 'global', baseUrl: 'http://global/v1', apiKey: 'global-key', models: ['g'] }]
    });

    expect(result.effectiveConfig.providers.map(p => p.name)).toEqual(['global', 'local']);
    const persisted = JSON.parse(readFileSync(join(TEST_DIR, 'config.json'), 'utf8'));
    expect(persisted.providers.map(p => p.name)).toEqual(['local']);
  });
});
