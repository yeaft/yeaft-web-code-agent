/**
 * task-327a: Yeaft LLM thinking/reasoning — capability matrix + adapter
 * parameter plumbing.
 *
 * Coverage:
 ...
 *   - Feature flag YEAFT_THINKING_V1 gates everything — default OFF is noop
 *
 * Red lines:
...
describe('task-327a: router.filterEffortForModel', () => {
  let origFlag;
  beforeEach(() => { origFlag = process.env.YEAFT_THINKING_V1; });
  afterEach(() => {
    if (origFlag === undefined) delete process.env.YEAFT_THINKING_V1;
    else process.env.YEAFT_THINKING_V1 = origFlag;
  });
  ...
  it('keeps effort when flag on + supported model', () => {
    process.env.YEAFT_THINKING_V1 = '1';
    const out = filterEffortForModel({ model: 'claude-opus-4-20250514', effort: 'max' });
...
describe('task-327a: AnthropicAdapter body.thinking injection', () => {
  let origFlag;
  beforeEach(() => {
    origFlag = process.env.YEAFT_THINKING_V1;
    process.env.YEAFT_THINKING_V1 = '1';
  });
  afterEach(() => {
    if (origFlag === undefined) delete process.env.YEAFT_THINKING_V1;
...
// Phase 7: ChatCompletionsAdapter was deleted. Its body.reasoning injection
// behaviour is fully covered by the OpenAIResponsesAdapter suite below.
