...
import { _resetVpBridgeForTest } from '../../../agent/yeaft/vp/vp-bridge.js';
...
function latestCrudResult(op, requestId) {
  return [...(ctx.ws.sent || [])]
    .reverse()
    .find(msg => msg.type === 'yeaft_output'
...
function latestSnapshot() {
  return [...(ctx.ws.sent || [])]
    .reverse()
    .find(msg => msg.type === 'yeaft_output' && msg.event?.type === 'vp_snapshot')?.event;
}
...
describe('vp-bridge — configured VP library', () => {
  ...
  it('reads, updates, and deletes VPs from the configured library', () => {
    ...
    handleYeaftVpRead({ type: 'yeaft_vp_read', requestId: 'read-1', vpId: 'custom_vp_01' });
