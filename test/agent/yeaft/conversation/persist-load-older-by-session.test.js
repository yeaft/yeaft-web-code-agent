/**
 * persist-load-older-by-group.test.js — pagination cursor for Yeaft
 * "Load older messages".
 *
 * Covers the contract documented on `ConversationStore.loadOlderBySession`:
...
describe('loadOlderBySession — empty / defensive paths', () => {
  it('returns empty result for missing/empty sessionId', () => {
    seedTurns('g1', 5);
    expect(store.loadOlderBySession(null, null, 5))
      .toEqual({ messages: [], oldestSeq: null, hasMore: false });
    expect(store.loadOlderBySession('', null, 5))
      .toEqual({ messages: [], oldestSeq: null, hasMore: false });
    expect(store.loadOlderBySession(undefined, null, 5))
      .toEqual({ messages: [], oldestSeq: null, hasMore: false });
  });

  it('returns empty result when group has no messages', () => {
    seedTurns('g-other', 3);
    const r = store.loadOlderBySession('g-empty', null, 5);
  ...
  it('treats beforeSeq=0 as "nothing older than seq 0"', () => {
    seedTurns('g1', 3);
    const r = store.loadOlderBySession('g1', 0, 5);
    expect(r.messages).toEqual([]);
    expect(r.oldestSeq).toBeNull();
    expect(r.hasMore).toBe(false);
  });
  ...
  it('treats beforeSeq=null/undefined/Infinity as "from newest"', () => {
    seedTurns('g1', 2);
    const fromNewestNull   = store.loadOlderBySession('g1', null, 50);
    const fromNewestUndef  = store.loadOlderBySession('g1', undefined, 50);
    const fromNewestInf    = store.loadOlderBySession('g1', Infinity, 50);
...
describe('loadOlderBySession — hot-only history', () => {
  it('returns the last N turns when called with no cursor', () => {
    seedTurns('g1', 5);
    const r = store.loadOlderBySession('g1', null, 2);
  ...
  it('hasMore=false when the slice covers the whole prefix exactly', () => {
    seedTurns('g1', 3);
    const r = store.loadOlderBySession('g1', null, 3);
  ...
  it('hasMore=false when turnsLimit > available turns', () => {
    seedTurns('g1', 2);
    const r = store.loadOlderBySession('g1', null, 10);
    expect(r.messages.map(m => m.content)).toEqual([
      'q1', 'aq1', 'q2', 'aq2',
    ]);
    expect(r.hasMore).toBe(false);
...
describe('loadOlderBySession — turn-boundary correctness', () => {
  it('21-turn group with turns=10 splits cleanly across 3 pages', () => {
    seedTurns('g1', 21);
    // Page 1 — newest 10 turns: q12..q21.
    const p1 = store.loadOlderBySession('g1', null, 10);
    ...
    const p2 = store.loadOlderBySession('g1', p1.oldestSeq, 10);
    ...
    const p3 = store.loadOlderBySession('g1', p2.oldestSeq, 10);
...
describe('loadOlderBySession — sessionId isolation', () => {
  it('ignores messages from other groups', () => {
    ...
    const a = store.loadOlderBySession('g_a', null, 10);
    expect(a.messages.map(m => m.content)).toEqual([
      'A1', 'aA1', 'A2', 'aA2', 'A3', 'aA3',
    ]);
    expect(a.hasMore).toBe(false);

    const b = store.loadOlderBySession('g_b', null, 10);
...
describe('loadOlderBySession — hot/cold tier crossing', () => {
  ...
  it('cold ids are strictly less than hot ids (chronology invariant)', () => {
    ...
    const all = store.loadOlderBySession('g1', null, 10);
...
describe('loadOlderBySession — race-with-streaming safety', () => {
  it('cursor remains stable when newer messages are appended after read', () => {
    seedTurns('g1', 3);
    const p1 = store.loadOlderBySession('g1', null, 1);
    ...
    const p2 = store.loadOlderBySession('g1', p1.oldestSeq, 1);
    expect(p2.messages.map(m => m.content)).toEqual(['q2', 'aq2']);
  });
});

describe('loadVisibleBySession — internal marker filtering', () => {
  it('keeps visible messages that quote internal frontmatter markers in the body', () => {
    ...
    const r = store.loadVisibleBySession('g1', null, 10);
    expect(r.messages.map(m => m.content)).toEqual([
