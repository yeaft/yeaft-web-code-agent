/**
 * store.test.js — group-isolated memory layout.
 */
...
describe('store: SCOPE_KINDS contract', () => {
  it('exposes the group-isolated scope kinds, frozen', () => {
    expect(SCOPE_KINDS).toEqual([
      'user', 'group', 'group-user', 'group-vp', 'group-feature', 'group-topic', 'chat', 'chat-vp', 'session', 'session-user', 'session-vp', 'session-feature', 'session-topic',
    ]);
