/**
 * group-update-op.test.js — handleYeaftUpdateGroup WS handler.
 *
...
import { handleYeaftUpdateGroup } from '../../../../agent/yeaft/web-bridge.js';
...
describe('handleYeaftUpdateGroup WS op', () => {
  let yeaftDir;
  beforeEach(() => {
  ...
  it('updates announcement and emits group_crud_result + snapshot broadcast', () => {
    handleYeaftUpdateGroup({ requestId: 'r1', groupId: 'g1', patch: { announcement: 'Hi' } });
  ...
  it('rejects empty patch with invalid_patch', () => {
    handleYeaftUpdateGroup({ requestId: 'r4', groupId: 'g1', patch: {} });
    const result = findEvent('group_crud_result');
    expect(result.ok).toBe(false);
  ...
  it('rejects missing patch with invalid_patch', () => {
    handleYeaftUpdateGroup({ requestId: 'r5', groupId: 'g1' });
    const result = findEvent('group_crud_result');
    expect(result.ok).toBe(false);
    expect(result.error.code).toBe('invalid_patch');
  });
