/**
 * attachments.test.js — Yeaft attachment plumbing.
 *
...
describe('persistYeaftAttachments', () => {
  let tmp;
  let prevCwd;

  beforeEach(() => {
    tmp = mkdtempSync(join(tmpdir(), 'yeaft-att-'));
    prevCwd = process.cwd();
    process.chdir(tmp);
  });
  ...
  it('returns empty bundle when no files', () => {
    const r = persistYeaftAttachments([], { subdir: 'grp_x' });
  ...
  it('writes a non-image file to disk under the group subdir', () => {
    const r = persistYeaftAttachments([
      {
        name: 'notes.md',
  ...
  it('caps attachments per turn at MAX_FILES_PER_TURN, surfacing excess in `failed`', () => {
    ...
    const r = persistYeaftAttachments(tooMany, { subdir: 'grp_cap' });
  ...
  it('caps total bytes at MAX_TOTAL_BYTES, surfacing oversize in `failed`', () => {
    ...
    const r = persistYeaftAttachments(files, { subdir: 'grp_bytes' });
...
describe('appendMessage rejects ephemeral leaks (structural guard)', () => {
  ...
  let tmp;
  beforeEach(() => {
    tmp = mkdtempSync(join(tmpdir(), 'yeaft-grp-'));
  });
  afterEach(() => {
    rmSync(tmp, { recursive: true, force: true });
  });
