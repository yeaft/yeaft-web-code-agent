...
describe('GroupContext.history — per-group isolation', () => {
  ...
  it('hydrate-from-disk: per-group records are isolated by groupId', () => {
    const dir = mkdtempSync(join(tmpdir(), 'unify-history-iso-'));
    try {
  ...
  it('lazy hydrate runs even if a partial entry was seeded earlier', async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), 'unify-history-iso-lazy-'));
    try {
      const store = new ConversationStore(dir);
  ...
  it('setGroupHistory(_, []) marks hydrated so consolidate does not reload', () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), 'unify-history-iso-consolidate-'));
    try {
      const store = new ConversationStore(dir);
