/**
 * YeaftSettings.js — task-343.
 *
 * Two-column tab dialog mirroring SettingsPanel sizing/tab tokens:
 *   Tab 1 (LLM)     → <LlmTab context="yeaft"> + task-318 runtime settings
 ...
 *
 * Agent scoping:
 *   Provider CRUD writes target store.yeaftAgentId. LlmTab normally binds
 *   to chatStore.currentAgent; the `context="yeaft"` prop swaps its
 *   effective agent id to yeaftAgentId without polluting Chat path.
 *
...
export default {
  name: 'YeaftSettings',
  components: { LlmTab, VpCrudPanel, SearchSettingsTab },
  props: {
    visible: { type: Boolean, default: true },
    initialTab: { type: String, default: 'llm' }, // 'llm' | 'vp' | 'search'
  },
  emits: ['close', 'saved'],
  template: `
    <div class="settings-overlay" v-if="visible" @click.self="$emit('close')">
      <div class="settings-dialog">
        ...
        <div class="settings-nav">
          <div class="settings-nav-title">{{ $t('yeaft.settings.title') }}</div>
          <button
            v-for="tab in tabs"
            :key="tab.key"
          ...
          >
            ...
            <svg v-else-if="tab.key === 'search'" viewBox="0 0 24 24" width="18" height="18"><path fill="currentColor" d="M15.5 14h-.79l-.28-.27A6.471 6.471 0 0 0 16 9.5 6.5 6.5 0 1 0 9.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z"/></svg>
            <span>{{ tab.label }}</span>
          </button>
        </div>
        ...
        <div class="settings-content">
          ...
          <div class="settings-scroll">
            ...
            <div v-show="activeTab === 'llm'" class="settings-pane">
              <LlmTab context="yeaft" @message="onLlmMessage" @saved="onLlmSaved" />
            ...
            <!-- Search pane: web-search backend + Tavily key + quota -->
            <div v-show="activeTab === 'search'" class="settings-pane">
  ...
  setup(props, { emit }) {
    ...
    const activeTab = Vue.ref(['llm', 'vp', 'search'].includes(props.initialTab) ? props.initialTab : 'llm');
    Vue.watch(() => props.initialTab, (next) => {
      if (['llm', 'vp', 'search'].includes(next)) activeTab.value = next;
    });

    const tabs = Vue.computed(() => [
      { key: 'llm', label: $t('yeaft.settings.tabs.llm') },
      { key: 'vp',  label: $t('yeaft.settings.tabs.vp') },
      { key: 'search', label: $t('yeaft.settings.tabs.search') },
    ]);
    ...
    // LlmTab saved → dispatch yeaft_reset so Engine picks up new config,
    // and propagate `saved` event to host (preserves task-275 behavior).
    function onLlmSaved() {
      const agentId = store.yeaftAgentId;
      if (agentId) {
        store.sendWsMessage({ type: 'yeaft_reset', agentId });
      }
      emit('saved');
    }

    // H2.f.6: Yeaft runtime settings (maxConcurrentThreads + autoArchiveIdleDays)
