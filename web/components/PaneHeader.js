    ...
    <div class="pane-header" v-if="conversationId">
      ...
      <div class="pane-header-actions">
        ...
        <template v-if="!isCrew">
          <button class="header-action-btn" @click="toggleExpertPanel" :title="$t('chatHeader.expertPanel')" v-if="!isCrew">
            <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              <path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/>
            </svg>
          </button>
          <button class="header-action-btn" @click="compactContext" :title="$t('chatHeader.compact')">
            <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
        ...
        <!-- Crew mode actions -->
        <template v-if="isCrew">
          <button class="header-action-btn" @click="toggleCrewRoles" title="Roles">
        ...
        <!-- Refresh (shared) -->
        <button class="header-action-btn" @click="refreshConversation" :title="$t('chatHeader.refresh')">
          <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
      ...
      <div class="pane-header-right">
        <button class="header-action-btn ph-add-btn" v-if="paneCount < 3" @click="$emit('add-pane')" :title="$t('splitScreen.addPane')">
          <svg viewBox="0 0 24 24" width="14" height="14"><path fill="currentColor" d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z"/></svg>
        </button>
        <button class="header-action-btn ph-close-btn" @click="$emit('close-pane')" :title="$t('splitScreen.closePane')">
