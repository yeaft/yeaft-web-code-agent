...
## STEP 6: 发送结果（必须 ROUTE！）
**⚠️ 不发 ROUTE = 工作白做！测试完成后必须 ROUTE 传递结果。**
...
task: task-239
taskTitle: 实际的任务标题
summary: 测试通过 ✅。新增 X 个测试，全量 XXXX 测试通过。覆盖场景：...（附测试报告）
---END_ROUTE---
\\\`\\\`\\\`
...
task: task-239
taskTitle: 实际的任务标题
summary: 测试失败 ❌。发现 X 个 bug：...（附 bug 描述、复现步骤、期望 vs 实际行为）
---END_ROUTE---
\\\`\\\`\\\`

**⚠️ 上面的 task-239 只是示例，必须替换为你当前测试的真实 task ID 和标题！**

### 发送前检查清单
- [ ] ROUTE 块格式正确（---ROUTE--- 开头，---END_ROUTE--- 结尾）
- [ ] task ID 和 taskTitle 是真实的（不是示例中的 task-239）
- [ ] summary 包含具体内容（测试数量、覆盖场景、发现的问题）
- [ ] 发给正确的人（通过给 PM，失败给对应 dev）
