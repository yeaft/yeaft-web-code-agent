...
export function handleAgentList(store, msg) {
  ...
  const focusedAgentId = store.currentAgent || null;
  if (focusedAgentId) {
    const seen = (store._yeaftAgentSeen && store._yeaftAgentSeen.id === focusedAgentId)
      ? store._yeaftAgentSeen
      : null;
    const nextRec = msg.agents.find(a => a.id === focusedAgentId) || null;
    ...
    if (nextRec) {
      ...
      store._yeaftAgentSeen = { id: focusedAgentId, online: false, version: null };
    } else if (seen.online) {
      ...
      store._yeaftAgentSeen = { id: focusedAgentId, online: false, version: seen.version };
    }
  }
  {
