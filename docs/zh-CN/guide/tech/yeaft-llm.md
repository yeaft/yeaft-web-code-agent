...
改完 `~/.yeaft/config.json` 不用重启 Agent：
- 设置 → Yeaft / LLM tab → **Reload config**
- 或调 Agent API：`POST /api/yeaft/reload-config`

reload 会：重新 parse config → 重建 AdapterRouter → 新建的 turn 用新配置（in-flight turn 用旧配置直到完成）。
