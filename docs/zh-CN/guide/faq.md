...
只能用 Copilot CLI 暴露的模型 —— 当前是 Claude 系（Sonnet 4 / 4.5）和 GPT 系（4.1 / 5 等）。需要别的厂商就用 Yeaft 会话，在 `~/.yeaft/config.json` 里加 provider。
