...
- **Yeaft 引擎的 `github-copilot` credential provider** — 这是 Yeaft Code Agent 用的 — 由 Yeaft 自己拿 GitHub OAuth token 直接调 Copilot API，**不**启动 `copilot --acp` 子进程

两者用同一个 GitHub 账号鉴权，但走不同的代码路径。前者面向 "我想用 Copilot CLI 替代 Claude CLI"，后者面向 "我想让 Yeaft Code Agent 里的某个 VP 用 Copilot 拿到的模型"。
