...
Yeaft 把三种 AI 后端摆在同一个 Web 界面里，你可以按任务选择。三者**不互斥** — 同一台 Agent 上可以同时跑 Claude Code 会话、Copilot 会话和 Yeaft Group，互不干扰。
...
| **Yeaft Group Mode** | 多角色并行讨论、跨任务持久记忆、自定义 VP（人格 + 模型 + 工具） | 单线程一对一对话（这是 Group Mode，不是 1:1） |
...
### Yeaft Group Mode（自有引擎的多 VP 群组协作）

- 不依赖任何外部 CLI — Yeaft 自带 query loop、记忆、工具
- 一个 group 里可以放多个 **VP（Virtual Person）** — 每个 VP 有自己的人格、模型、工具集
- 一个用户消息可以**并行 fan-out** 到多个 VP，每个 VP 同时回复
- **H2-AMS 持久记忆** — 跨会话保留 VP / group / user / feature 级 scope
...
侧边栏顶部 tab bar 切到 **Yeaft**，再用 `+` 建 group：

1. 输入 group 名
2. 添加 VP（每个 VP 选 model、可选 persona、可选工具白名单）
3. 把 user message 发到 group，VP 们会**并行**响应
...
- **想让多个 VP 长期记住你 / 跨任务延续记忆 / 自由组合 OpenAI + Anthropic** → Yeaft Group Mode
