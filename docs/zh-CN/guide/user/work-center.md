...
看板保留「进行中 / 需要处理 / 已关闭」三栏，用轻量竖线分区，不给整列增加卡片外框。列标题、数量、卡片和空状态统一对齐，卡片使用紧凑字号；空间不足时切换为单阶段标签。顶栏右侧提供搜索、筛选、刷新和直接打开设置的齿轮图标；窄屏搜索收纳到筛选面板。「＋ 新建工作项」位于独立侧栏顶部，桌面折叠后保留加号，窄屏从导航抽屉进入。创建窗口可选择在线 Agent，默认沿用当前选择；切换时保留需求和交付目标说明，重置目录、执行时间及附件，并加载所选 Agent 的默认值与历史候选，避免串用其他 Agent 的执行环境。
...
创建时在「执行时间」选择**立即开始、保存草稿或定时执行**。定时表单使用随主题变化的内联日历和 24 小时时间输入，不依赖系统日期弹层；选择时区后，预览展示该时区的首次执行时间。

- **仅一次**：到期启动这个工作项。
- **每天 / 工作日 / 每周 / 每月**：保存独立的执行计划，每轮生成新的工作项，保留各轮对话、Actions、结果和用量。工作日指周一至周五，不含节假日调休规则；每周可多选星期，每月可选 1–31 日，短月份取月末。
- **结束条件**：持续重复、截至指定日期（所选时区当天结束），或生成指定次数（1–1,000 次）。次数按实际生成的工作项计算，不要求该轮成功。

计划详情显示状态、下次执行时间、时区和已生成次数，可跳转到最近一轮；每轮也可返回来源计划。暂停只停止生成后续工作项，不中断已开始的工作。恢复重复计划时跳过暂停期间的时段。取消计划不会取消已经生成的工作项；已结束或取消的计划需重新创建。

调度由 Agent 执行，关闭浏览器不影响它。Agent 离线后恢复只合并处理最近一个已到时段，不集中补跑所有历史；同一计划的上一轮尚未完成或取消时，跳过本轮以避免重叠。夏令时跳过不存在的本地时刻，回拨时同一时刻只执行一次。重复计划会持续消耗各轮执行资源，确认目标和频率后再创建。

旧 Agent 未声明支持重复计划时，界面保留单次定时并禁用重复选项。一次性计划暂停后若已过期，需要新的未来时间才能恢复；当前界面可重新创建该单次计划。
