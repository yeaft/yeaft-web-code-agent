...
Agent 是一个跑在你自己机器（笔记本、VPS、开发容器）上的 Node.js 进程，通过 WebSocket 连到 Yeaft 服务器。**一个 Agent 进程能同时处理三种后端** —— Claude Code、Copilot、Yeaft Group Mode —— 具体哪几个可用，取决于本机装了哪些 CLI。

## 前置要求

机器需要 Node.js 18+。下表的 CLI **都不是必须的** —— 你想用哪种后端就装哪个：
...
| **Yeaft Group Mode** | **无** —— 引擎随 npm 包一起发；你只需要在 `~/.yeaft/config.json` 里配好至少一个 LLM provider | 见 [Yeaft 引擎配置](./yeaft-config.md) |
...
- **Yeaft Group** 选项始终在 —— 引擎已经打包好了，但实际跑聊天前需要先在 `~/.yeaft/config.json` 里配至少一个 provider
