...
The board keeps **In progress / Needs attention / Closed** columns with light vertical separators rather than nested column cards. Titles, counts, compact cards, and empty states share consistent alignment. Narrow workspaces switch to single-stage tabs. Search, filters, refresh, and a gear that opens settings directly share the header; narrow screens move search into filters. **+ New work item** sits near the top of the dedicated sidebar, remaining a plus icon in the desktop rail and accessible through the navigation drawer on narrow screens. The create dialog lets you choose an online Agent and defaults to the current selection. Switching keeps the requirement and delivery goal text, but resets the directory, execution timing, and attachments and loads the chosen Agent's defaults and history, rather than carrying another Agent's execution context across.
