...
- **Yeaft engine's `github-copilot` credential provider** — used by Yeaft Group Mode; Yeaft grabs the GitHub OAuth token itself and calls the Copilot API directly, **no** `copilot --acp` subprocess

Both authenticate against the same GitHub account, but they take different code paths. The first is for "I want Copilot CLI as a Claude replacement"; the second is for "I want a VP inside Yeaft Group to use Copilot-provided models".
