...
- 已完成日程不可编辑。取消源计划后投影日程状态为 `cancelled`，停止生成，不允许通过任何启动/恢复接口重新启动该计划；已有实例不被取消。需要再次重复执行时创建新计划。
...
日历计算独立于执行调度。夏令时导致本地时间不存在时跳过当天；时钟回拨产生两个相同本地时间时仅选择较早的那个。SQLite 可嵌套的同步 savepoint 在一个事务里完成到期检查、生成实例、入队启动、推进日程（首次写入时取得写锁，内层释放不提交外层事务）；唯一 `(sourceScheduleId, scheduledOccurrenceAt)` 索引进一步防止重启或并发扫描重复生成。事务失败不消耗次数。
...
存储迁移为 schema 42（保留主线 schema 41 的交付目标迁移），采用新增列和索引，保留所有一次性日程和历史执行结果。

## 调度失败可见性

`lastError` 为 `null` 或 `{ code: "schedule_dispatch_failed", message, at }`。失败状态由既有 events 持久恢复，`at` 是本次连续失败首次出现的时间，不是最后重试时间。同一连续失败阶段只记录/广播一次，避免每次扫描刷屏；成功触发、推进或显式修改日程后清除。失败事件携带源计划身份和递增 revision；浏览器展示双语通用提示，不透传原始异常、路径或凭据。
