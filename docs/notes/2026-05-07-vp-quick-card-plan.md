...
**目标：** 把 Yeaft 群聊里 `intent === 'feature'` 的 VP turn 从主消息流的 `AssistantTurn` 完整展开，改成紧凑的 `VpQuickCard` 卡片；点卡片在右侧滑出 `VpTurnDetailDrawer`，里面复用 `AssistantTurn` 渲染完整 messages + tools。

**架构：**
- `MessageList.turnGroups` 在已有的 turn 对象上派生 `intent`（`store.yeaftQuickPreviews[vpId:turnId]?.intent || 'quick'`）。模板按 `intent` 分流：feature → 新组件 `VpQuickCard`，其他 → 现有 `AssistantTurn`（不动）。
- 新增 store state `yeaftOpenVpTurnDetail: { vpId, turnId } | null` + 两个 action。`YeaftPage.js` 监听这个 state 渲染 `VpTurnDetailDrawer`。drawer 主体直接挂 `<AssistantTurn :turn="targetTurn" />`，零内容重写。
...
**关键已知前提（不动）：**
- Track-A wire 协议、`store.yeaftQuickPreviews` 的写入、`injectQuickPreviews` 的调用都已在跑。本计划不动 backend，不改 wire。
- `AssistantTurn` 接收 `:turn` 一个 prop（看 `web/components/AssistantTurn.js:9-18`），可以无包装直接复用为 drawer body。
- Yeaft 当前 NOT 持久化到 SQLite `messages` 表（`server/handlers/agent-output.js:318` 是纯转发），重载 web 时不会回灌 quick preview——所以**本计划不做持久化**。重载后 feature turn 会因 `yeaftQuickPreviews` 为空而 fallback 到 `intent='quick'`（设计文档已锁定的安全降级）。持久化作为独立 follow-up plan。
...
| `web/styles/yeaft-vp-card.css` | `VpQuickCard` 样式（紧凑 chip-like）+ 状态指示器配色。 |
| `web/styles/yeaft-detail-drawer.css` | drawer 右侧定位 + 滑入动画 + info popover 浮层样式。 |
...
| `test/web/vp-turn-detail-drawer.test.js` | drawer 显隐由 `yeaftOpenVpTurnDetail` 驱动；info popover 切换。 |
| `test/web/turn-intent-injection.test.js` | turnGroups 输出在每个 `assistant-turn` 上正确写入了 `intent`（依赖 `store.yeaftQuickPreviews`）。 |

### 修改
| 文件 | 改动 |
|---|---|
| `web/stores/chat.js` | 加 state `yeaftOpenVpTurnDetail`；加 actions `openVpTurnDetail({vpId,turnId})` / `closeVpTurnDetail()`。 |
...
| `web/components/YeaftPage.js` | 顶层加 `<VpTurnDetailDrawer v-if="store.yeaftOpenVpTurnDetail" />`。引入 css 文件。 |
 ...
 *
 * The web store keeps a `yeaftQuickPreviews` map keyed by `${vpId}:${turnId}`
 ...
 * @param {object|null} previewMap — `store.yeaftQuickPreviews`
...
git commit -m "feat(yeaft): add deriveTurnIntent pure helper for VP card render branch"
```
...
```javascript
            currentTurn.showSpeakerHeader = !!currentTurn.speakerVpId;
            currentTurn.intent = deriveTurnIntent(currentTurn, store.yeaftQuickPreviews);
            result.push(currentTurn);
```
...
git commit -m "feat(yeaft): stamp intent on every assistant-turn in turnGroups"
```
...
- 创建: `web/styles/yeaft-vp-card.css`
 ...
 *   preview — store.yeaftQuickPreviews entry: { intent, preview, ... } | null
 *
 * Emits:
...
```bash
git add web/components/VpQuickCard.js web/styles/yeaft-vp-card.css \
        web/i18n/en.js web/i18n/zh-CN.js test/web/vp-quick-card.test.js
git commit -m "feat(yeaft): add VpQuickCard with status indicator for feature turns"
```

---

## 任务 4: chat.js store — yeaftOpenVpTurnDetail state + actions

**文件：**
- 修改: `web/stores/chat.js`（state 块加 `yeaftOpenVpTurnDetail: null`，actions 块加 `openVpTurnDetail` / `closeVpTurnDetail`）
...
describe('chat.js — yeaftOpenVpTurnDetail', () => {
  beforeEach(() => setActivePinia(createPinia()));

  it('starts at null', () => {
    const store = useChatStore();
    expect(store.yeaftOpenVpTurnDetail).toBeNull();
  });

  it('openVpTurnDetail sets the descriptor', () => {
    const store = useChatStore();
    store.openVpTurnDetail({ vpId: 'jobs', turnId: 't1' });
    expect(store.yeaftOpenVpTurnDetail).toEqual({ vpId: 'jobs', turnId: 't1' });
  });
  ...
  it('openVpTurnDetail with another vpId/turnId switches detail target', () => {
    ...
    expect(store.yeaftOpenVpTurnDetail).toEqual({ vpId: 'wozniak', turnId: 't2' });
  });
...
打开 `web/stores/chat.js`，在 state 块里 `yeaftQuickPreviews: {},`（约 311 行）下面找 `// Active VP turns`（约 313 行）。在那里、相同区域追加：
...
```javascript
    ...
    // `yeaftActiveVpDetailId` (the older center-column persona view) —
...
在 actions 块（找 `enterYeaft(agentId = null)` 旁边的兄弟 actions），追加两个新 action（位置随便选个语义相近的；`enterYeaft` 之前/之后都可以）：
...
```javascript
    ...
    openVpTurnDetail(payload) {
      ...
      this.yeaftOpenVpTurnDetail = { vpId: payload.vpId, turnId: payload.turnId };
    },

    closeVpTurnDetail() {
      this.yeaftOpenVpTurnDetail = null;
    },
```
...
git commit -m "feat(yeaft): add yeaftOpenVpTurnDetail state + open/close actions"
```
...
- 创建: `web/styles/yeaft-detail-drawer.css`
...
describe('VpTurnDetailDrawer', () => {
  ...
  function mountWithStore({ openTarget, messages }) {
    ...
    if (messages && store.yeaftConversationId) {
      store.messagesMap[store.yeaftConversationId] = messages;
    } else if (messages) {
      store.yeaftConversationId = 'yeaft-test';
      store.messagesMap['yeaft-test'] = messages;
    }
  ...
  it('renders nothing when yeaftOpenVpTurnDetail is null', () => {
 ...
 *
 * Activation: store.yeaftOpenVpTurnDetail = { vpId, turnId } | null.
...
export default {
  ...
  computed: {
    target() {
      return this.store.yeaftOpenVpTurnDetail;
    },
    targetTurn() {
      const t = this.target;
      if (!t) return null;
...
预期: PASS（5 tests）。如果某个测试因为找不到 `useChatStore` 中 `yeaftConversationId` 的初始值，先 grep 看 store 默认值，必要时在测试 `mountWithStore` 里手动 set。

- [ ] **步骤 6: 提交**

```bash
git add web/components/VpTurnDetailDrawer.js web/styles/yeaft-detail-drawer.css \
        test/web/vp-turn-detail-drawer.test.js
git commit -m "feat(yeaft): add VpTurnDetailDrawer with in-drawer info popover"
```
...
<VpQuickCard
  ...
  :turn="item"
  :preview="store.yeaftQuickPreviews[item.speakerVpId + ':' + item.turnId]"
...
describe('MessageList — VpQuickCard render branch', () => {
  ...
  it('renders VpQuickCard when a turn is intent=feature', async () => {
    const store = useChatStore();
    store.yeaftConversationId = 'yeaft-test';
    store.activeConversations = ['yeaft-test'];
    store.messagesMap['yeaft-test'] = [
      { type: 'assistant', content: 'partial...', vpId: 'jobs', turnId: 't1',
        speakerVpId: 'jobs', isStreaming: true },
    ];
    store.yeaftQuickPreviews = {
...
git commit -m "feat(yeaft): branch MessageList between VpQuickCard and AssistantTurn by intent"
```

---

## 任务 7: YeaftPage 挂载 drawer + 引入 css

**文件：**
- 修改: `web/components/YeaftPage.js`（顶层模板加 `<VpTurnDetailDrawer />`，import 组件）
- 修改: `web/index.html`（`<link>` 引入两个新 css）

- [ ] **步骤 1: 修改 YeaftPage.js**

打开 `web/components/YeaftPage.js`。找顶部 import 块加：
...
模板里找最外层 wrapper（搜 `<div class="yeaft-page"` 或类似根 div），紧挨着 `</template>` 之前的位置（即根 div 的最后一个子元素）插入：
...
打开 `web/index.html`，在 `<head>` 已有的 `<link rel="stylesheet" href="styles/yeaft.css">` 等行旁边追加：

```html
<link rel="stylesheet" href="styles/yeaft-vp-card.css">
<link rel="stylesheet" href="styles/yeaft-detail-drawer.css">
```
...
不需要重启 server。如果本地有 dev 环境：访问 Yeaft 页面，发 `@SomeVP 帮我做个复杂任务`，观察主流是 VpQuickCard，点击后右侧 drawer 滑出，drawer 里能看到完整 messages + tool use；点 ⓘ 出现 popover；点 ✕ 关闭 drawer；切换另一个 VP 卡片 drawer 内容更新。

发 `@SomeVP 你好` 这种简单消息，应该走老路（AssistantTurn 完整展开）。

如果手动冒烟有问题（典型：VpQuickCard 没出现，AssistantTurn 还在显示）—— 先 console.log 看 `store.yeaftQuickPreviews` 是否有 entry，再核对 `intent === 'feature'` 是否被正确派生。**不要**绕过测试直接 patch；回到任务 2 / 3 加针对性测试再修。

- [ ] **步骤 5: 提交**

```bash
git add web/components/YeaftPage.js web/index.html
git commit -m "feat(yeaft): mount VpTurnDetailDrawer in YeaftPage; load new CSS"
```
...
gh pr create --base main --head <worktree-branch> \
  --title "feat(yeaft): VP quick card + right-side detail drawer" \
  --body "$(cat <<'EOF'
## Summary
...
- 持久化 quick_preview / quick_intent —— Yeaft 当前不走 SQLite messages 表，需要先建立 jsonl 重载路径，是独立工作

## Test plan
- [ ] 新增 5 个测试文件全 PASS
- [ ] `npx vitest run` 全量绿
- [ ] 手动：Yeaft 群聊里 `@vp 复杂任务` 出现 VpQuickCard；点开 drawer；info popover 切换；切换 VP 卡片切换 drawer 内容
