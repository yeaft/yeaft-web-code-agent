...
export class Engine {
  ...
  async #runOrchestratorCompact(budget, _compactCfg) {
    ...
    const summariserSystem = isZh
      ? '你是对话摘要器。下面包含「先前累计摘要」（可能为空）与「新待压缩对话」。请融合两者，输出一份「重写后的累计摘要」——不要分段罗列日期、不要保留 "## 2026-..." 等历史分节，直接产出一份连贯、可被未来轮次直接 append 进 prompt 的摘要。保留关键决策、事实、上下文与人物意图。'
    ...
    // Pull the *current* on-disk summary so the next pass rewrites it in
    // place rather than producing a parallel snippet. Without this,
    // earlier compact rounds would silently fall off when we overwrite
    // the file with only the new round's output. The summariser sees
    // both sides and is instructed (above) to merge into one document.
