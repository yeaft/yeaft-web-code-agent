...
1. **群 = 会话**. The "session" concept is weakened. One group is one continuous conversation; opening Yeaft drops you into a group. Single-VP "chat" is just a degenerate group with one VP.
...
These live in `agent/yeaft/templates/harness/*.md` and are concatenated by `buildWorkerPrompt`. **Ship in Phase 1**, not as a follow-up.
...
1. **Pinning tests** (already started in `test/agent/yeaft/vp-prompt-and-routing.test.js`). Extend to cover:
...
1. **UI setting.** A `Think: high ▾ / max` selector lives in the Yeaft topbar next to the model picker. Two modes:
   ...
   The UI value is sent on the `yeaft_session_chat` WebSocket frame as `thinking: 'high' | 'max'` and reaches the dispatcher in `submitOptions`.
...
**UI plumbing:**
- Pinia store: `yeaftThinking: 'high' | 'max'` (transient) + `yeaftThinkingLocked: boolean`.
- The selector emits an `yeaft_session_chat` frame with `thinking` set; the agent's `web-bridge.handleYeaftSessionChat` puts it into `submitOptions.queryOpts.thinking` (per-VP via `runVpTurn`); the dispatcher records it in the route-decision precedence as the highest-priority signal.
- Lock toggle calls a new `yeaft_group_set_thinking` WS message that writes `groups/<g>/meta.md`. Reads on group load.
