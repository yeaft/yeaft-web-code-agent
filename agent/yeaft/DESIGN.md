...
1. **UI setting.** A `Think: high ▾ / max` selector lives in the Yeaft topbar next to the model picker. Two modes:
   ...
   The UI value is sent on the `yeaft_group_chat` WebSocket frame as `thinking: 'high' | 'max'` and reaches the dispatcher in `submitOptions`.
...
- The selector emits an `yeaft_group_chat` frame with `thinking` set; the agent's `web-bridge.handleYeaftGroupChat` puts it into `submitOptions.queryOpts.thinking` (per-VP via `runVpTurn`); the dispatcher records it in the route-decision precedence as the highest-priority signal.
