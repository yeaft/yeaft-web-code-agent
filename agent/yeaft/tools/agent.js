...
Async orchestration:
  ...
  3. ListAgents  — non-blocking status check when you need progress info.
  4. WaitAgent   — short poll (<5s) or explicit long wait only when you truly
                    need the result before your next reply.
  5. CloseAgent  — tear down the sub-agent when done.

Never call WaitAgent in a loop without a terminal condition. If the sub-agent
is still running after a short check, move on and let notifications inform you
next turn.`,
